...
i64 GetTickCountTh()
{
	...
	PrevTime = curTime;
	if( TimeSpeed != 1.0 && (is(GameMode, GM_NORMAL, GM_HARDCORE, GM_IRONMAN, GM_NIGHTMARE, GM_EASY, GM_SURVIVAL, GM_HARD, GM_SPEEDRUN) && MaxCountOfPlayersInGame == 1
	 || DevelopMode) ){
...
void __fastcall Character_Passive_Life_and_Mana_Regeneration( int playerIndex ) // life & mana regeneration (passive)
{
	...
	if (isMageArchetype && player.CurMana > player.MaxCurMana) {
		// mana above max (from potions/mana charges/magi charges) decays back toward max at a flat 2% of max mana per second
		int manaOverflowDecayPerTick = player.MaxCurMana * 2 / (100 * ENGINE_FPS);
